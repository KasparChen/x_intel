...
class CryptoBot:
    ...
    async def get_id(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(f"当前群组/频道 ID: {chat_id}")
    ...
    async def handle_button(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "add_admin":
            ...
            await query.edit_message_text("请输入频道ID, eg -100123456789", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("返回", callback_data="back")]]))
            context.user_data["action"] = "add_receive_channel"
        elif data == "query_receive_channel":
            await query.edit_message_text(f"当前接收频道：{', '.join(self.receive_channels)}")
    ...
    async def perform_summarize(self, update, context, reset_cycle):
        ...
        if not messages:
            await update.message.reply_text("无新消息")
            return
    ...
    async def handle_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if action == "add_admin":
            ...
            if text in self.admins:
                self.admins.remove(text)
                await update.message.reply_text(f"已移除管理员：{text}")
        elif action == "add_receive_channel":
            self.receive_channels.append(text)
            await update.message.reply_text(f"已添加接收频道：{text}")
        elif action == "set_review_channel":
            self.review_channel = text
            await update.message.reply_text(f"审核频道设置为：{text}")
        elif action == "set_publish_channel":
            self.publish_channel = text
            await update.message.reply_text(f"发布频道设置为：{text}")
    ...
    async def receive_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        log_info(f"收到消息: {message['content'][:20]}...")
    ...
    async def send_review(self, context, summary):
        ...
        await context.bot.send_message(self.review_channel, summary, reply_markup=InlineKeyboardMarkup(keyboard))
